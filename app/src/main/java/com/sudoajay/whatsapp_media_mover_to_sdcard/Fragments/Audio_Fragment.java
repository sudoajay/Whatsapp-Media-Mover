...
public class Audio_Fragment extends Fragment {
    ...
    public Audio_Fragment createInstance(After_MainTransferFIle after_main_transferFIle, String which_Option_To_Do, List<File> selected_List
    , Context context) {
    ...
    private void run_Selected() {
        ...
        if (which_Option_To_Do.equals("restore"))
            make_changes = new Make_Changes(after_main_transferFIle.getSd_Card_Path_URL() + whats_App_Media_Path +
                    whatsapp_Path.substring(1, whatsapp_Path.length()-1)+" Audio/");
        else
